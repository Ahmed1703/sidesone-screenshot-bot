...
function buildPromptOverrideFromWriting(basePrompt, writing, leadData = {}) {
  ...
  // --- Length (controls how many findings to weave AND total sentence budget) ---
  const lengthMap = {
    short: "Write a short email. 4 sentences total including the greeting. WEAVE EXACTLY 1 finding (the most severe one across visual/mobile/seo). Around 50 words.",
    medium: "Write a normal-length email. 7 sentences total including the greeting. WEAVE EXACTLY 2 findings, ideally from different categories (e.g. one visual + one mobile, or one mobile + one SEO). Around 100 words.",
    long: "Write a detailed email. 11 sentences total including the greeting. WEAVE EXACTLY 3 findings spanning the categories you have material for. Around 180 words.",
  };
  const findingCountMap = { short: 1, medium: 2, long: 3 };
  const targetFindingCount = findingCountMap[writing.outputLength] || 2;
  ...
  lines.push(
    seed,
    "",
    "SOURCE OF FACTS — READ THIS FIRST.",
    "The WEBSITE ANALYSIS JSON below contains a `findings` object with three categories: `visual`, `mobile`, and `seo`. Each finding has an `issue`, a `consequence`, and a `severity` (low/medium/high). These have been verified by the system from real screenshots and a real DOM scrape — they are facts, not guesses. Use them as your raw material. Do not invent issues that are not in findings. Do not soften the consequences.",
    "",
    `FINDING SELECTION — pick exactly ${targetFindingCount} finding${targetFindingCount === 1 ? "" : "s"} to weave into this email.`,
    "Selection rules, in order:",
    "1) Prefer high severity over medium over low. Never skip a `high` to include a `medium` or `low`.",
    `2) When picking ${targetFindingCount > 1 ? targetFindingCount + " findings" : "1 finding"}, ${targetFindingCount > 1 ? "favor variety across categories — visual, mobile, seo. A mix lands harder than three findings of the same flavor." : "just take the single most severe finding regardless of category."}`,
    "3) If a category is empty in findings, skip it silently. NEVER mention that the SEO is good, NEVER say \"mobile looks fine\", NEVER announce a category at all. The reader does not know there were three categories.",
    "4) If the total available findings across all categories is fewer than the target, write a SHORTER email rather than padding. A 4-sentence email with one sharp finding beats a 7-sentence email with two padded ones.",
    "",
    "EMAIL STRUCTURE:",
    "",
    "GREETING + HOOK (2 sentences max).",
    hasFirst
      ? `Greet with "${recipientFirst}".`
      : "Simple greeting.",
    hasSender
      ? "Your name comes FIRST right after the greeting. Then jump straight into a CONCRETE thing you noticed on the site — something a real person could verify in 3 seconds (a logo, a headline, a hero image subject, a section name, a color, a button placement). Like \"snublet over siden deres\" + a specific observation. NOT a generic statement about what websites should do."
      : "Open with a CONCRETE thing you noticed on the site — something a real person could verify in 3 seconds (a logo, a headline, a hero image subject, a section name, a color, a button placement). NOT a generic statement about what websites should do.",
    "BANNED OPENINGS (do not write anything resembling these):",
    "  - \"Nettsiden deres må fungere godt for å...\" / \"Your website needs to...\"",
    "  - \"Det er viktig at en nettside...\" / \"It is important that a website...\"",
    "  - \"For at folk skal kontakte dere...\" / \"For people to contact you...\"",
    "  - \"Et godt førsteinntrykk er...\" / \"A good first impression is...\"",
    "  - Any sentence that could be sent unchanged to a different company.",
    "DO NOT open with fake compliments either: \"siden ser grei ut\" / \"the site looks fine\" / \"nice site\". The opening must mention something specific about THIS site that proves you actually looked at it.",
    "",
    "BODY — WEAVE THE FINDINGS.",
    "Take your selected findings and write them as ONE flowing thought, not a list. Each finding becomes 1-2 sentences in the email: the issue (in your own words, plain language) followed by the consequence (in your own words, as a real human reaction). Connect them with natural bridges: \"som gjør at\", \"så\", \"og det betyr at\", \"which means\", \"so\", \"and that makes\". Vary the phrasing — do not overuse \"noe som\" or \"which means\".",
    "Move between findings with natural transitions: \"I tillegg\", \"Også\", \"En annen ting\", \"On top of that\", \"And the other thing\". Never label a category. The reader should not be able to guess that the findings came from three buckets.",
    "WHAT A CONSEQUENCE SOUNDS LIKE — say it the way a person says it out loud. \"Folk tør ikke fylle det ut\" not \"reduces form completion\". \"Folk finner deg ikke når de googler røykinspeksjon i Bergen\" not \"poor SEO ranking\". \"Halvparten av besøkende er på telefon og må zoome\" not \"non-responsive design\".",
    "MOBILE consequences sound like: phone visitors having to pinch and zoom, the menu being broken on small screens, the CTA being hidden below the fold, text being too small to read on a phone.",
    "SEO consequences sound like: not showing up when someone Googles your service in your city, the Google snippet being cut off or showing garbled text, social shares having no preview image so they look like spam.",
    "VISUAL consequences sound like: people leaving before they read anything, the eye not knowing where to land, the site feeling abandoned so people assume the business is too, the form feeling so cold no one fills it out.",
    "",
    "EXAMPLES of the right shape. Do NOT copy. Write your own.",
    "  GOOD (visual): \"Hovedbildet er grøtete og lavoppløselig, så det første inntrykket blir at siden ikke er helt å stole på.\"",
    "  GOOD (mobile): \"På telefon må folk zoome og dra siden sidelengs for å lese, og da lukker de fleste fanen før de finner ut hva dere driver med.\"",
    "  GOOD (seo): \"Tittelen til siden er bare domenet, så når noen googler tjenesten deres i Bergen kommer dere ikke opp.\"",
    "  GOOD (mixed): \"The hero text sits on a busy background so the eye doesn't know where to land, and on top of that, on a phone the whole layout forces you to zoom — most people just close the tab.\"",
    "BAD examples. Do not write like this.",
    "  BAD: \"The form is ugly.\" No consequence.",
    "  BAD: \"Your SEO is not optimal.\" Vague, no human impact.",
    "  BAD: \"On the SEO side, your meta description is too long.\" Announces the category. Sounds like a checklist.",
    "  BAD: \"Your site looks fine on desktop but the SEO is great.\" Filler praise, breaks the human voice.",
    "",
    "NEVER mention the navigation menu. Skip it entirely.",
    "NEVER start the body with \"Det første som slår meg\" or \"The first thing I noticed\". Just start.",
    "",
    "CLOSING. 1 to 2 sentences.",
    "Transition naturally from the last finding into your closing goal. Acknowledge briefly that what you described is fixable — then make the offer. Write it in your own words. Different every time. Do not list the offer mechanically; weave it into a thought.",
    "",
    "SIGN-OFF.",
    ...
    "",
    "PARAGRAPHING — REQUIRED.",
    "Use blank lines (one empty line, two newlines) between paragraphs. The body must NEVER come out as one wall of text.",
    "Structure:",
    "  - The greeting (\"Hei,\" / \"Hei [name],\") is always its own short line/paragraph.",
    "  - The body is 1 or 2 paragraphs. Use 1 if all findings flow naturally as one thought. Split into 2 only if there is a real shift in topic between findings (e.g. visual/feel beat, then a search-visibility beat). Do NOT put each finding on its own line — that turns the email back into a list and breaks the weaving rule.",
    "  - The closing/offer is always its own paragraph.",
    "Per length tier:",
    "  - Short emails: 2-3 paragraphs total (greeting / body+closing fused, OR greeting / body / closing).",
    "  - Medium emails: 3 paragraphs typically (greeting / body / closing).",
    "  - Long emails: 3-4 paragraphs (greeting / body, optional second body for the topic shift, / closing).",
    "Hard caps: max 4 sentences per paragraph. If a paragraph would run longer, split at the natural topic shift.",
    ""
  );
  ...
  lines.push(
    "WRITING STYLE.",
    "- Sound like a real person typing a quick email at 11pm. Warm, curious, interested in helping. NEVER like a UX consultant, a slide deck, a LinkedIn post, or a textbook.",
    "- Plain everyday words only. If a word sounds like something a regular person would not say out loud over coffee, do not use it.",
    "",
    "ABSOLUTE LANGUAGE RULE — NO FOREIGN WORDS.",
    "When writing in Norwegian, every single word must be a Norwegian word. Zero English words, ever. Common slips to avoid:",
    "  - WRONG: \"desktop-bredde\". RIGHT: \"PC-bredde\" or \"vanlig PC-versjon\".",
    "  - WRONG: \"responsive\" / \"mobile-first\" / \"user-friendly\".",
    "  - WRONG: \"meta description\" / \"viewport\" / \"alt-text\" / \"SEO\" / \"call to action\".",
    "  - RIGHT for those things: \"den korte teksten Google viser under tittelen\", \"så siden viser seg riktig på telefon\", \"beskrivelse av bildene\", \"når folk googler tjenesten\", \"den knappen folk skal klikke på\".",
    "Same rule for every other language. If you reach for an English technical word, rewrite the sentence in plain everyday terms a small business owner would say out loud.",
    "",
    "BANNED CORPORATE VOCABULARY — Norwegian.",
    "  Never use: \"fungere godt\", \"fange oppmerksomhet\", \"miste tillit\", \"brukervennlig\", \"oppleves som\", \"helhetsinntrykk\", \"førsteinntrykk\" used vaguely, \"moderne og ryddig\", \"profesjonelt uttrykk\", \"konvertering\", \"engasjement\", \"synlighet\" as a buzzword, \"kvalitet\" as vague praise, \"besøkere\" (use \"folk\" or \"de som kommer innom\").",
    "BANNED CORPORATE VOCABULARY — English.",
    "  Never use: \"engage visitors\", \"first impression\", \"user experience\", \"professional appearance\", \"effective\", \"impactful\", \"streamlined\", \"clean and modern\", \"brand identity\", \"conversion\", \"engagement\", \"visibility\" as a buzzword.",
    "If you reach for these words, rewrite using a CONCRETE observation a friend would say. \"Designet føles uprofesjonelt\" → \"Logoen ser ut som den er klippet rett ut av Word\". \"Reduces user trust\" → \"folk tror dere har sluttet å bruke siden i 2014\".",
    "",
    ...
    "- Concrete observations only. Visual: text size, button visibility, image quality, spacing, colors, forms. Mobile: zooming, broken nav, hidden CTA, tiny tap targets. SEO: invisible in search for the service, garbled snippet, no preview when shared. No vague filler.",
    "- Every problem must be tied to a real human consequence. Someone leaving. Not trusting the site. Not daring to fill a form. Not clicking. Not calling. Not finding the business when they search. Never leave an issue floating without naming who it hurts and how.",
    "- Write the consequence the way a friend would say it. \"Folk tør ikke fylle det ut\". Not \"reduces form completion rates\". \"People bounce\". Not \"causes elevated exit rates\". \"Folk finner deg ikke når de googler\". Not \"poor SEO ranking\". \"Folk lurer på om dere finnes lenger\". Not \"reduces brand trust\".",
    ...
    "- Never mention screenshots, AI, tools, JSON, the analysis, scraping, the DOM, meta tags by name, or anything technical that a small business owner would not say out loud.",
    "- Don't describe what the company does. Critique the website only — visual, mobile, and search visibility.",
